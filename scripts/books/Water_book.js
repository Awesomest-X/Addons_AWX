...
world.beforeEvents.worldInitialize.subscribe(initEvent => {
  initEvent.itemComponentRegistry.registerCustomComponent('awx:water_book', {
    ...
    onUseItem(e) {
      ...
      let storedItems = player.getComponent('water_book_storage').get('awx:water_storage') || [];
      ...
      if (storedItems.length > 0) {
        ...
        player.getComponent('water_book_storage').set('awx:water_storage', []);

        // Action Bar message informing the player
        player.sendActionBarMessage("You Have Opened The Book Of Water!");
      } else {
...
function storeItems(player, item) {
  ...
  for (let slot = 0; slot < inventory.size; slot++) {
    ...
    if (slotItem && itemsStored < 7) {
      ...
      if (slotItem.getTags().includes('awx:water_page')) {
  ...
  player.getComponent('water_book_storage').set('awx:water_storage', storedItemData);
