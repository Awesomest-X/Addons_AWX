...
world.beforeEvents.worldInitialize.subscribe(initEvent => {
  initEvent.itemComponentRegistry.registerCustomComponent('awx:fire_book', {
    ...
    onUseItem(e) {
      ...
      let storedItems = player.getComponent('fire_book_storage').get('awx:fire_storage') || [];
      ...
      if (storedItems.length > 0) {
        ...
        player.getComponent('fire_book_storage').set('awx:fire_storage', []);
...
function storeItems(player, item) {
  ...
  player.getComponent('fire_book_storage').set('awx:fire_storage', storedItemData);
