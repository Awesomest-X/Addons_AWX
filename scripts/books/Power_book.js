...
world.beforeEvents.worldInitialize.subscribe(initEvent => {
  initEvent.itemComponentRegistry.registerCustomComponent('awx:power_book', {
    ...
    onUseItem(e) {
      ...
      let storedItems = player.getComponent('power_book_storage').get('awx:power_storage') || [];
      ...
      if (storedItems.length > 0) {
        ...
        player.getComponent('power_book_storage').set('awx:power_storage', []);

        // Action Bar message informing the player
        player.sendActionBarMessage("You Have Opened The Book Of Power!");
      } else {
...
function storeItems(player, item) {
  ...
  for (let slot = 0; slot < inventory.size; slot++) {
    ...
    if (slotItem && itemsStored < 7) {
      ...
      if (slotItem.getTags().includes('awx:power_page')) {
  ...
  player.getComponent('power_book_storage').set('awx:power_storage', storedItemData);
