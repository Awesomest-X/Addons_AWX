...
world.beforeEvents.worldInitialize.subscribe(initEvent => {
  initEvent.itemComponentRegistry.registerCustomComponent('awx:earth_book', {
    ...
    onUseItem(e) {
      ...
      let storedItems = player.getComponent('earth_book_storage').get('awx:earth_storage') || [];
      ...
      if (storedItems.length > 0) {
        ...
        player.getComponent('earth_book_storage').set('awx:earth_storage', []);
...
function storeItems(player, item) {
  ...
  for (let slot = 0; slot < inventory.size; slot++) {
    ...
    if (slotItem && itemsStored < 7) {
      ...
      if (slotItem.getTags().includes('awx:earth_page')) {
  ...
  player.getComponent('earth_book_storage').set('awx:earth_storage', storedItemData);
