...
world.beforeEvents.worldInitialize.subscribe(initEvent => {
  initEvent.itemComponentRegistry.registerCustomComponent('awx:storm_book', {
    ...
    onUseItem(e) {
      ...
      let storedItems = player.getComponent('storm_book_storage').get('awx:storm_storage') || [];
      ...
      if (storedItems.length > 0) {
        ...
        player.getComponent('storm_book_storage').set('awx:storm_storage', []);

        // Action Bar message informing the player
        player.sendActionBarMessage("You Have Opened The Book Of Storm!");
      } else {
...
function storeItems(player, item) {
  ...
  for (let slot = 0; slot < inventory.size; slot++) {
    ...
    if (slotItem && itemsStored < 7) {
      ...
      if (slotItem.getTags().includes('awx:storm_page')) {
  ...
  player.getComponent('storm_book_storage').set('awx:storm_storage', storedItemData);
